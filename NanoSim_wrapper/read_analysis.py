...
Modified in June 2018 by Lucile

@author: Chen Yang

This script generates read profiles Oxford Nanopore reads.
...
###############################################################################
# Modification: only use minimap2 to align Long Reads
###############################################################################    
    else:
        file_extension = "sam"
        out_sam = prefix + ".sam"
        sys.stdout.write(strftime("[RNanoSim] %Y-%m-%d %H:%M:%S") + ": Alignment with minimap2\n")
        #call(aligner+ " --cs -ax map-ont " + ref + " " + in_fasta + " > " + out_sam, shell=True)
        print(aligner+ " --cs -ax splice -k14 -uf " + ref + " " + in_fasta + " > " + out_sam)
        call(aligner+ " --cs -ax splice -k14 -uf " + ref + " " + in_fasta + " > " + out_sam, shell=True)
...
###############################################################################
# Modification: no 'unaligned reads'
###############################################################################  
